...
function makeList(chartDataList) {
    ...
    let category = $("#categorySelect").val();
    var prev5min = new Date(new Date().setMinutes(new Date().getMinutes() - 5))
    var prev2hour = new Date(new Date().setHours(new Date().getHours() - 2))
    for (var i in chartDataList) {
        chartDataList[i]['isReceive'] = ''
        if (category == 'airkor') {
            console.log(prev2hour)
            console.log(new Date(chartDataList[i].sensor * 1000))
            console.log(new Date(chartDataList[i].sensor * 1000) < prev2hour)
            chartDataList[i]['isReceive'] = (new Date(chartDataList[i].timeStamp * 1000) < prev2hour) || (chartDataList[i].timeStamp == undefined) ? '미수신' : '수신'

        }
        else if(category == 'sdot' || category == 'kweather'){
            chartDataList[i]['isReceive'] = (new Date(chartDataList[i].timestamp * 1000) < prev5min) || (chartDataList[i].timestamp == undefined) ? '미수신' : '수신'
            ...
            var convertTm = getDetailMapDateInNewDate(chartDataList[i].sensor.tm)
            chartDataList[i]['isReceive'] = (new Date(convertTm) < prev5min) || (chartDataList[i].sensor.tm == undefined) ? '미수신' : '수신'
            ...
            chartDataList[i]['isReceive'] = (new Date(chartDataList[i].sensor.timestamp * 1000) < new Date(prev5min)) || (chartDataList[i].sensor.timestamp == undefined) ? '미수신' : '수신'
...
function getCSV(filename) {
    var csv = [];
    var row = [];
    row.push("지점명", "지점번호", "위도", "경도","수신현황");
    csv.push(row.join(","));

    $.each(chartDataList, function (index, data) {
        row = [];
        row.push(data.stationName, data.serial, data.lat, data.lon, data.isReceive);
        csv.push(row.join(","));
    });
